(function () {
  ...
  if (!stepBtn || !rack || !stepsList) return;
  ...
  function stepOnce() {
    if (idx >= steps.length) { clearAll(); return; }
    ...
    const offset = 60 + (ctrl ? ctrl.offsetHeight : 0) + (rackWrap ? rackWrap.offsetHeight : 0) + 16;
    const y = steps[idx].getBoundingClientRect().top + window.pageYOffset - offset;
