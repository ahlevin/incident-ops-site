(function () {
  ...
  async function runAll() {
    ...
    for (let i = 0; i < CHG_DEMO_ENTRIES.length; i++) {
      renderEntry(CHG_DEMO_ENTRIES[i]);
      if (i < CHG_DEMO_ENTRIES.length - 1) {
    ...
    stepBtn.textContent = "Step through";
    stepIndex = CHG_DEMO_ENTRIES.length;
    busy = false;
  }
  ...
  function stepOnce() {
    ...
    if (stepIndex >= CHG_DEMO_ENTRIES.length) {
      resetConsole();
      runBtn.textContent = "Run all";
      return;
    }

    renderEntry(CHG_DEMO_ENTRIES[stepIndex]);
    stepIndex++;

    if (stepIndex >= CHG_DEMO_ENTRIES.length) {
      stepBtn.textContent = "Start over";
    } else {
      stepBtn.textContent = "Next step (" + (stepIndex + 1) + " of " + CHG_DEMO_ENTRIES.length + ")";
    }
  }
